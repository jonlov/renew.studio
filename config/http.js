    ...
    middleware: {
        ...
        pathPolicy: function(req, res, next) {
            ...
            var url = req.path,
                ...
                noAuth = function () {
                    return require('../api/services/publicPaths.js')
                };

            function pathPolicy() {
                if (noAuth().urls.indexOf(url) > -1) return false;
                if (noAuth().folders.indexOf(firstParam) > -1)
                    return false;

                if (noAuth().subFolders.indexOf(filePath) > -1)
                    return false;
