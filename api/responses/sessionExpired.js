...
module.exports = function forbidden(data, options) {
    ...
    if (!req.wantsJSON) {
        ...
        req.options.locals.status = 419;
        req.options.locals.title = 'Expired Session | ' + config.companyName;
    }
    // Set status code
    res.status(419);
    ...
    if (data !== undefined) {
        sails.log.verbose('Sending 419 ("Expired Session") response: \n', data);
    } else sails.log.verbose('Sending 419 ("Expired Session") response');
